 ...
 * @dev    v1.8.2 — Bug fix: constructor now whitelists NATIVE_ETH (address(0)) in
 *         supportedTokens so that native DC can be used in createInvoice,
 *         approveRecurring, and transferToUser. Was silently blocked in v1.7.0
 *         when the constructor was simplified and the ETH whitelist line was lost.
 * @dev    v1.8.1 — Audit follow-up: M-4 residual closed — adminRefundToPayer now
 *         restricted to DISPUTED/CHALLENGE_PENDING only, eliminating the partial-
 *         refund path that let a rogue employee redirect escrow to the merchant
 *         before any dispute or confirmation window; N-2 emergency ERC-20 sweep
 *         now validates the returned bool in addition to revert status, matching
 *         SafeERC20 semantics; stale markComplete NatSpec corrected (dueDate guard
 *         added by M-3 was not reflected in the docstring).
 * @dev    v1.8.0 — Security audit fixes: M-1 removeExternalWallet permissionless;
 *         M-2 isFamilyTransfer requires recipient opt-in via approveFamilySender();
 *         M-3 markComplete blocked after dueDate; M-4 adminReleaseToMerchant
 *         restricted to DISPUTED/CHALLENGE_PENDING, onlyAdmin; M-5 emergency ERC-20
 *         sweep failure-isolated; M-6 nextDueDate anchored to block.timestamp;
 *         L-1 renounceOwnership disabled; L-2 Ownable2Step; L-3 payerAcknowledged
 *         enforced in triggerRecurring; L-4 PREPAID+recurring rejected; L-7 dead
 *         nonces removed; I-1 postpaid dueDate guard; I-6 redundant event removed.
 * @dev    v1.7.0 — Open platform: merchant registration and subscription system
 *         removed. Any wallet can now create invoices without admin approval.
 *         Constructor simplified to a single parameter (defaultFeeBps); all tokens
 *         must be added after deployment via addSupportedToken().
 ...
 */
contract CryptoPaymentPlatform is Ownable2Step, ReentrancyGuard, Pausable {
    ...
    /// @dev recipient → sender → whether recipient has opted the sender in as family.
    mapping(address => mapping(address => bool)) public familyApproved;
    ...
    // Feature 4 — Family sender opt-in
    event FamilySenderApproved(address indexed recipient, address indexed sender, bool approved);
    ...
    /**
     * @notice Deploys the platform and sets the default percentage-based fee.
     *         No tokens are whitelisted at deploy time — call addSupportedToken()
     *         after deployment to enable each payment token.
     *
     * @param defaultFeeBps Initial global fee in basis points (e.g. 250 = 2.5 %).
     */
    constructor(uint256 defaultFeeBps) Ownable(msg.sender) {
        ...
        // Native ETH is always supported; whitelisted here because addSupportedToken()
        // blocks address(0) to prevent accidental overwrites after deployment.
        ...
        returns (uint256 invoiceId)
    {
        ...
        if (isRecurring && paymentType == PaymentType.PREPAID) revert InvalidAmount();
     ...
     *
     * @dev    Only valid on PAID status before the invoice dueDate. Once dueDate
     *         passes, the payer may call reclaimFunds() instead (M-3).
     *
        ...
        invoiceExists(invoiceId)
    {
        ...
        inv.completedCycles++;
        inv.nextDueDate      = block.timestamp + inv.recurringInterval;
    ...
    {
        Invoice storage inv      = _invoices[invoiceId];
        // Only genuine dispute statuses — prevents bypassing payer's normal dispute window.
        if (inv.status != InvoiceStatus.DISPUTED &&
            inv.status != InvoiceStatus.CHALLENGE_PENDING) {
            revert InvalidInvoiceStatus(invoiceId, inv.status);
        }
    ...
    {
        Invoice storage inv      = _invoices[invoiceId];
        // Restricted to dispute statuses only — prevents a rogue employee from
        // using the partial-refund branch to redirect escrow to the merchant
        // before any dispute window has been entered (M-4 residual fix).
        if (inv.status != InvoiceStatus.DISPUTED &&
            inv.status != InvoiceStatus.CHALLENGE_PENDING) {
            revert InvalidInvoiceStatus(invoiceId, inv.status);
        }
    ...
    /**
     * @notice Emits `AdminTransferred` whenever ownership actually changes hands
     *         (covers both the Ownable2Step accept path and direct transfers).
     */
    ...
    /// @notice Disabled — renouncing would permanently brick admin functions
    ///         and trap all future fee income in address(0).
            ...
            for (uint256 j = 0; j < tokens.length; ) {
                ...
                if (bal > 0) {
                    ...
                    if (token == NATIVE_ETH) {
                        ...
                        (bool tokOk, bytes memory ret) = token.call(
                            abi.encodeWithSelector(IERC20.transfer.selector, user, bal)
                        );
                        // Treat as failure if the call reverted OR returned false (non-standard tokens).
                        bool tokSuccess = tokOk && (ret.length == 0 || abi.decode(ret, (bool)));
                        if (tokSuccess) {
                            emit Withdrawal(user, token, bal);
                        } else {
                            _ledger[user][token] = bal; // restore — skip this user/token
                        }
                    }
                }
                unchecked { j++; }
        ...
        for (uint256 k = 0; k < escrowInvoiceIds.length; ) {
            ...
            if (!esc.released && esc.amount > 0) {
                ...
                if (token == NATIVE_ETH) {
                    ...
                    (bool tokOk, bytes memory ret) = token.call(
                        abi.encodeWithSelector(IERC20.transfer.selector, payer, amount)
                    );
                    bool tokSuccess = tokOk && (ret.length == 0 || abi.decode(ret, (bool)));
                    if (tokSuccess) {
                        emit FundsRefunded(invoiceId, payer, amount);
                    } else {
                        _ledger[payer][token] += amount; // credit ledger as fallback
                    }
                }
        ...
        onlySupportedToken(token)
    {
        ...
        if (isFamilyTransfer) {
            // Recipient must have explicitly opted the sender in as family.
            if (!familyApproved[recipient][msg.sender]) revert Unauthorized();
    ...
    /**
     * @notice Recipient opts a sender in (or out) as an approved family sender,
     *         enabling fee-free transfers from that sender up to the monthly limit.
     *
     * @param sender   Wallet to approve or revoke.
     * @param approved true = allow fee-free; false = revoke.
     */
    function approveFamilySender(address sender, bool approved) external {
        if (sender == address(0))  revert ZeroAddress();
        if (sender == msg.sender)  revert CannotTransferToSelf();
        familyApproved[msg.sender][sender] = approved;
        emit FamilySenderApproved(msg.sender, sender, approved);
    }
